...
if (isset($_POST['saveSetting'])) {
    $stmt = $con->prepare("UPDATE users SET name = ?, number = ? WHERE id = ?");
    $stmt->bind_param("ssi", $_POST['name'], $_POST['number'], $_SESSION['userId']);
    ...
    if ($stmt->execute()) {
        ...
        $notice = "<div class='alert alert-danger'>Error: " . $stmt->error . "</div>";
    }
    $stmt->close();
}
