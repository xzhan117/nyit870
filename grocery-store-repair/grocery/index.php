...
if (isset($_POST['safeIn'])) 
{
  ...
  $fileType = mime_content_type($_FILES['inPic']['tmp_name']);
  if (!in_array($fileType, $allowedTypes)) {
    $notice = "<div class='alert alert-danger'>Invalid file type. Only JPEG, PNG, and GIF are allowed.</div>";
  } else {
    $uniqueFilename = uniqid() . "_" . basename($_FILES['inPic']['name']);
    $targetFilePath = "photo/" . $uniqueFilename;
    if (move_uploaded_file($_FILES["inPic"]["tmp_name"], $targetFilePath) &&
      $con->query("INSERT INTO categories (name, pic, details) VALUES ('$_POST[inName]', '$uniqueFilename', '$_POST[inDetails]')")) {
      $notice = "<div class='alert alert-success'>Successfully Saved</div>";
    } else {
       $notice = "<div class='alert alert-danger'>Error: Unable to save.</div>";
    }
  }
}
?>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body"> <form method="POST" enctype="multipart/form-data">
        <div style="width: 77%;margin: auto;">
          ...
          <div class="form-group">
            <label for="some" class="col-form-label">Details</label>
            <input type="text" name="inDetails" class="form-control" id="3" required>
          </div>
